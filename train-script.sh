...
python  -m "knockoff.victim.train"  CIFAR10 resnet18 -d 0 \
        -o $SOURCEDIR/models/victim/cifar10-resnet18 -e 1 --log-interval 25
printf  "\n\nend of victim.train\n\n"

python -m "knockoff.adversary.transfer" random $SOURCEDIR/models/victim/cifar10-resnet18 \
        --out_dir $SOURCEDIR/models/adversary/cifar10-resnet18-random --budget 1000 \
        --queryset ImageNet1k $SLURM_TMPDIR/ILSVRC2012 --batch_size 10 -d 0

printf  "\n\nend of adversary.transfer\n\n"

python -m "knockoff.adversary.train" $SOURCEDIR/models/adversary/cifar10-resnet18-random \
