...
class OpenImages(ImageFolder):
    ...
    def __init__(self, train=True, transform=None, target_transform=None):
        root = osp.join(cfg.DATASET_ROOT, 'openImages')
        if not osp.exists(root):
            raise ValueError('Dataset not found at {}. Please download it from {}.'.format(
                root, 'https://storage.googleapis.com/openimages/web/index.html'
            ))
    ...
    def get_partition_to_idxs(self):
        ...
        # Note: we perform a 80-20 split of openimages training
